...
pub fn scan() -> anyhow::Result<Vec<ScanInfo>> {
    // 1. Find the Risk window from all the active windows.
    let active_windows = xcap::Window::all()?;
    let risk_window = active_windows
        .iter()
        .find(|w| w.title() == "RISK")
        .cloned()
        .ok_or(anyhow::anyhow!("Unable to find Risk window."))?;

    // 2. Screenshot and crop the player cards from the RISK window.
    let scrshot_path = paths::scrshot_path()
        .ok_or(anyhow::anyhow!("Unable to construct screenshot path."))?;

    scrshot_window(&risk_window, &scrshot_path)?; // This saves the RISK screenshot to `scrshot_path`.
    crop_player_cards_1920_1080(&scrshot_path)?;

    // 3. Create the OCR engine, load the images, and extract the text.
    ...
    // 4. Load the blacklist.
    let blacklist_path = paths::blacklist_path().ok_or(anyhow::anyhow!("Unable to construct blacklist path."))?;
    let blacklist = blacklist::Blacklist::load(&blacklist_path).expect("Unable to load the blacklist.");

    // 5. Fuzzy match the detections against the blacklist.
    let mut scans: Vec<ScanInfo> = Vec::new();
    for detection_text in detections.iter()
    {
        // 5.1. Detections with a length less than 1 are probably false positives.
...
fn crop_player_cards_1920_1080(scrshot_path: &PathBuf) -> anyhow::Result<()> {
...
fn create_ocr_engine() -> anyhow::Result<OcrEngine> {
